...
then
	...
	then
		...
		FLATPAK_APP_LIST=`/usr/bin/flatpak list --app --columns=application | /usr/bin/grep -v 'Application ID' | /usr/bin/awk -F '.' '{ if ( $NF == "Client" ) print $2; else print $NF; }' | /usr/bin/tr "[A-Z]" "[a-z]"`
